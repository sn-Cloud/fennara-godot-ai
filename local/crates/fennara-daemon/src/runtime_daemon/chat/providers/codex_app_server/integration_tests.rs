...
fn multiple_godot_project_scopes_keep_codex_bindings_isolated() {
    ...
    let first_chat = store::create_chat(&first_scope, "codex/gpt-5.5-codex", "medium", &[])
        .expect("create first editor chat");
    let second_chat = store::create_chat(&second_scope, "codex/gpt-5.5-codex", "medium", &[])
    ...
    assert!(
        !first_chats
            .iter()
            .any(|chat| chat.id == second_chat.chat.id)
    );
    assert!(
        second_chats
            .iter()
    ...
    assert!(
        !second_chats
            .iter()
            .any(|chat| chat.id == first_chat.chat.id)
    );
