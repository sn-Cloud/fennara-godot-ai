...
- `sync-doc-navigation.mjs`: adds source hashes, stable anchors, and the compact same-page language selector without translating prose.
- `check-doc-i18n.mjs`: validates complete locale coverage, source freshness, navigation, anchors, Markdown structure, protected code, URLs, and links.
- `doc-i18n-lib.mjs`: owns the shared locale manifest, source normalization, navigation rendering, and structural helpers.
...
The locale and document set is declared in `docs/i18n/languages.json`. English
remains canonical. Translated prose must be written from the English source,
not generated by these scripts.

Normal sync updates navigation and stable anchors but preserves existing source
hashes. After directly updating all nine translations for a changed English
page, deliberately refresh only that source:

```bash
node scripts/sync-doc-navigation.mjs --accept-source docs/cli.md
```

The option may be repeated for several reviewed sources. Do not acknowledge a
source whose translated prose has not been updated. CI runs
`sync-doc-navigation.mjs --check` before the full translation validator.
